...
\includepdf[scale=0.76,pages=1,pagecommand=\section{System architecture}]{./img/architecture.pdf}
...
The first step is to crop the image to only contain the region of interest (ROI). This is a camera setting that can be predefined so that the camera only grabs the ROI and thus it is not needed to crop it after the frame is grabbed.\\
